...
{
    /// <summary>
    /// Suporte oficial para apertar Play direto em CenaMapaNOVO no Editor.
    /// Garante singletons minimos, restaura a equipe do save e sobe um host local limpo.
    /// </summary>
    [DefaultExecutionOrder(-10000)]
    public class CenaMapaNOVODirectPlayBootstrap : MonoBehaviour
    {
        ...
        private GameDataManager PrepareGameData()
        {
            ...
            if (dataManager == null)
            {
                Debug.LogError("[CenaMapaNOVODirectPlayBootstrap] Nao foi possivel inicializar o GameDataManager.");
                return null;
            }
            ...
            if (!HasPlayableTeam(dataManager))
                Debug.LogError("[CenaMapaNOVODirectPlayBootstrap] Nenhuma equipe valida foi encontrada no save ou no fallback de debug.");
        ...
        private void ApplyFallbackSelection(GameDataManager dataManager)
        {
            ...
            Debug.LogWarning("[CenaMapaNOVODirectPlayBootstrap] Save sem equipe jogavel. Aplicando fallback de debug para o play direto.");
        }
        ...
        private void PrimeHostCharacterChoice(GameDataManager dataManager)
        {
            ...
            {
                Debug.LogError("[CenaMapaNOVODirectPlayBootstrap] Nao foi possivel resolver o comandante local na bibliotecaOriginalPersonagens.");
                return;
            }

            CharacterChoiceCache.SetHostCharacterIndex(characterIndex, "CenaMapaNOVODirectPlayBootstrap");
        }
        ...
        private IEnumerator StartLocalHostIfNeeded()
        {
            ...
            {
                Debug.LogError("[CenaMapaNOVODirectPlayBootstrap] NetworkManager.Singleton nao encontrado em CenaMapaNOVO.");
                yield break;
            }
            ...
            if (!started)
            {
                Debug.LogError("[CenaMapaNOVODirectPlayBootstrap] StartHost() falhou durante o play direto.");
                yield break;
            }

            Debug.Log("[CenaMapaNOVODirectPlayBootstrap] Host local iniciado para play direto em CenaMapaNOVO.");
        }
